...
void DemoInitialize()
{
	...
	for (i = 0; i < DISPLAY_NUM_FRAMES; i++)
	{
		...
		//framebuf = 0x11c370, size is 3*1080*1920
		//pFrames = 0x11c0a0
	}
...
void DemoRun()
{
	...
	char userInput = 0;
	int result;
	/*int *number[0] = (char *) 0x20000000;
	int *number[1] = (char *) 0x21000000;
	int *number[2] = (char *) 0x22000000;
	int *number[3] = (char *) 0x23000000;
	int *number[4] = (char *) 0x24000000;
	int *number[5] = (char *) 0x25000000;
	int *number[6] = (char *) 0x26000000;
	int *number[7] = (char *) 0x27000000;
	int *number[8] = (char *) 0x28000000;
	int *number[9] = (char *) 0x29000000;*/

	/* Flush UART FIFO锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷洗锟斤拷锟斤拷碌锟斤拷锟斤拷锟� */
	while (XUartPs_IsReceiveData(UART_BASEADDR))
		//UART_BASEADDR锟角存储锟斤拷锟斤拷锟斤拷锟捷的伙拷锟斤拷氐牡锟街�
	{
		XUartPs_ReadReg(UART_BASEADDR, XUARTPS_FIFO_OFFSET);
		//XUartPs_ReadReg锟斤拷锟斤拷锟斤拷取锟斤拷锟节斤拷锟秸碉拷锟斤拷锟捷ｏ拷锟斤拷锟捷伙拷锟节讹拷取锟斤拷锟斤拷锟斤拷锟皆讹拷锟斤拷锟�
	}
	...
	while (userInput != 'q')
	{
		...
		case '7':
			...
			result = VideoCatch(&videoCapt, pFrames[videoCapt.curFrame]);
			...
			DemoPrintTest(pFrames[dispCtrl.curFrame], dispCtrl.vMode.width, dispCtrl.vMode.height, DEMO_STRIDE, result);
			break;
		case '8':
			nextFrame = videoCapt.curFrame + 1;
			if (nextFrame >= DISPLAY_NUM_FRAMES)
...
void DemoPrintTest(u8 *frame, u32 width, u32 height, u32 stride, int pattern)
{
	...
	case DEMO_PATTERN_0:
		...
		for(xcoi = 0; xcoi < (width*3); xcoi+=3)
		{
			 ...
			 */
			iPixelAddr = xcoi;
			image_pixel_addr = xcoi;
			for(ycoi = 0; ycoi < height; ycoi++)
			{
				frame[iPixelAddr] = gImage_0[image_pixel_addr + 2];
				frame[iPixelAddr + 1] = gImage_0[image_pixel_addr + 1];
				frame[iPixelAddr + 2] = gImage_0[image_pixel_addr + 0];
		...
		for(xcoi = 0; xcoi < (width*3); xcoi+=3)
		{
			...
			for(ycoi = 0; ycoi < height; ycoi++)
			{
				frame[iPixelAddr] = gImage_1[image_pixel_addr + 2];
				frame[iPixelAddr + 1] = gImage_1[image_pixel_addr + 1];
				frame[iPixelAddr + 2] = gImage_1[image_pixel_addr + 0];
		...
		for(xcoi = 0; xcoi < (width*3); xcoi+=3)
		{
			...
			for(ycoi = 0; ycoi < height; ycoi++)
			{
				frame[iPixelAddr] = gImage_2[image_pixel_addr + 2];
				frame[iPixelAddr + 1] = gImage_2[image_pixel_addr + 1];
				frame[iPixelAddr + 2] = gImage_2[image_pixel_addr + 0];
		...
		for(xcoi = 0; xcoi < (width*3); xcoi+=3)
		{
			...
			for(ycoi = 0; ycoi < height; ycoi++)
			{
				frame[iPixelAddr] = gImage_3[image_pixel_addr + 2];
				frame[iPixelAddr + 1] = gImage_3[image_pixel_addr + 1];
				frame[iPixelAddr + 2] = gImage_3[image_pixel_addr + 0];
		...
		for(xcoi = 0; xcoi < (width*3); xcoi+=3)
		{
			...
			for(ycoi = 0; ycoi < height; ycoi++)
			{
				frame[iPixelAddr] = gImage_4[image_pixel_addr + 2];
				frame[iPixelAddr + 1] = gImage_4[image_pixel_addr + 1];
				frame[iPixelAddr + 2] = gImage_4[image_pixel_addr + 0];
		...
		for(xcoi = 0; xcoi < (width*3); xcoi+=3)
		{
			...
			for(ycoi = 0; ycoi < height; ycoi++)
			{
				frame[iPixelAddr] = gImage_5[image_pixel_addr + 2];
				frame[iPixelAddr + 1] = gImage_5[image_pixel_addr + 1];
				frame[iPixelAddr + 2] = gImage_5[image_pixel_addr + 0];
		...
		for(xcoi = 0; xcoi < (width*3); xcoi+=3)
		{
			...
			for(ycoi = 0; ycoi < height; ycoi++)
			{
				frame[iPixelAddr] = gImage_6[image_pixel_addr + 2];
				frame[iPixelAddr + 1] = gImage_6[image_pixel_addr + 1];
				frame[iPixelAddr + 2] = gImage_6[image_pixel_addr + 0];
		...
		for(xcoi = 0; xcoi < (width*3); xcoi+=3)
		{
			...
			for(ycoi = 0; ycoi < height; ycoi++)
			{
				frame[iPixelAddr] = gImage_7[image_pixel_addr + 2];
				frame[iPixelAddr + 1] = gImage_7[image_pixel_addr + 1];
				frame[iPixelAddr + 2] = gImage_7[image_pixel_addr + 0];
		...
		for(xcoi = 0; xcoi < (width*3); xcoi+=3)
		{
			...
			for(ycoi = 0; ycoi < height; ycoi++)
			{
				frame[iPixelAddr] = gImage_8[image_pixel_addr + 2];
				frame[iPixelAddr + 1] = gImage_8[image_pixel_addr + 1];
				frame[iPixelAddr + 2] = gImage_8[image_pixel_addr + 0];
		...
		for(xcoi = 0; xcoi < (width*3); xcoi+=3)
		{
			...
			for(ycoi = 0; ycoi < height; ycoi++)
			{
				frame[iPixelAddr] = gImage_9[image_pixel_addr + 2];
				frame[iPixelAddr + 1] = gImage_9[image_pixel_addr + 1];
				frame[iPixelAddr + 2] = gImage_9[image_pixel_addr + 0];
